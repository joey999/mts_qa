...
class HttpClient(object):
    def __init__(self, api_host, timeout=20):
    ...
    def request(self, endpoint, method='POST', data=None, headers=None, cookies=None):
        ...
        if not str(response.status_code).startswith('2'):
            ...
            utils.allure_attach('Bad status code. Status code:%s' % response.status_code,
                                'Error message:\n%s\n' % response.content)
    ...
    def write_log(url, **kwargs):
        ...
        with allure.step(as_curl):
            # helpers.allure_attach('curl', as_curl)
